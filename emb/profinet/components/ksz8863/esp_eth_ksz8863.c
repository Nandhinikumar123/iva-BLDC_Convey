...
esp_err_t ksz8863_sw_resett(esp_eth_handle_t port_eth_handle)
{
    esp_err_t ret = ESP_OK;
...
esp_err_t ksz8863_sw_reset(esp_eth_handle_t port_eth_handle)
{
    ...
    if (!eth->phy_reg_write) {
        ESP_LOGE(TAG, "phy_reg_write function pointer is NULL!");
    ...
    return eth->phy_reg_write(eth, 0, KSZ8863_RESET_ADDR, reset.val);
}

esp_err_t ksz8863_hw_resett(int reset_gpio_num)
{
    if (reset_gpio_num >= 0) {
...
esp_err_t ksz8863_hw_reset(int reset_gpio_num)
{
    if (reset_gpio_num >= 0) {
        ...
        // --- Assert reset ---
        gpio_set_level(reset_gpio_num, 0);
       vTaskDelay(pdMS_TO_TICKS(1000)); // Hold low for 50 ms

        // --- Deassert reset ---
        gpio_set_level(reset_gpio_num, 1);
      vTaskDelay(pdMS_TO_TICKS(1500)); // Wait for PHY to stabilize
    }
    return ESP_OK;
}
