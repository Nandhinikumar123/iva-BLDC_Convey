...
static void pf_ppm_drv_sw_send (pnet_t * net, void * arg, uint32_t current_time)
{
   ...
   uint32_t delay = 0;
 ESP_LOGE("PPM_SEND", "fired ci_running=%d", p_arg->ppm.ci_running);  // ADD
...
pnal_buf_t * send_buf = (pnal_buf_t *)p_arg->ppm.p_send_buffer;
int send_ret = pf_eth_send_on_physical_port (net, 1, send_buf);
ESP_LOGE("PPM_SEND", "eth_send ret=%d len=%u", send_ret, send_buf->len);
      /* Now send it */
      if (send_ret> 0)
      {
...
int pf_ppm_drv_sw_activate_req (pnet_t * net, pf_ar_t * p_ar, uint32_t crep)
{
   ...
   /* Send first frame immediately and synchronously */
   pf_ppm_finish_buffer (net, p_ppm, p_iocr->in_length);
   pnal_buf_t * send_buf = (pnal_buf_t *)p_ppm->p_send_buffer;
   pf_eth_send_on_physical_port (net, 1, send_buf);
   ESP_LOGE("PPM", "Immediate first send on activate");

   /* Then schedule recurring sends */
   ret = pf_scheduler_add (
      net,
      p_ppm->control_interval,   // back to normal interval
      pf_ppm_drv_sw_send,
      p_iocr,
      &p_ppm->ci_timeout);
