   ...
   uint8_t len)
{
   ...
   uint16_t tlv_header =
    ((typ << LLDP_TYPE_SHIFT) |
     (len & LLDP_LENGTH_MASK));
   pf_put_uint16 (true, tlv_header, PF_FRAME_BUFFER_SIZE, p_buf, p_pos);
}

static inline void pf_lldp_add_tlv_header_ez (
   uint8_t * p_buf,
   uint16_t * p_pos,
   uint8_t typ,
   uint8_t len)
...
static void pf_lldp_add_chassis_id_tlv (
   ...
   uint16_t * p_pos)
{
   // ESP_LOGE("EZHillllllllllllll", "BEFORE CHASSIS TLV: subtype=%d len=%d pos=%d",
   //       p_chassis_id->subtype,
   //       p_chassis_id->len,
   //       *p_pos);
   ...
   pf_put_mem (
      ...
      p_buf,
      p_pos);
      // ESP_LOGE("EZHillllllllllllll", "CHASSIS TLV: subtype=%d len=%d pos=%d",
      //    p_chassis_id->subtype,
      //    p_chassis_id->len,
      //    *p_pos);
}
   ...
   uint16_t * p_pos)
{
   ...
   pf_put_mem (
      ...
      p_buf,
      p_pos);
      // ESP_LOGE("EZHIL_PORT", "PORT TLV: subtype=%d len=%d pos=%d",
      //    p_port_id->subtype,
      //    p_port_id->len,
      //    *p_pos);
}
...
static void pf_lldp_add_ttl_tlv (uint8_t * p_buf, uint16_t * p_pos)
{
   ...
   //ESP_LOGE("Ezhillllllllll", "TTL TLV: ttl=%d pos=%d", PF_LLDP_TTL, *p_pos);
}
   ...
   uint16_t * p_pos)
{
   ...
   // ESP_LOGE("Ezhil......",
   //       "PORT STATUS: RT2=0x%04X RT3=0x%04X pos=%d",
   //       rtclass_2_status,
   //       rtclass_3_status,
   //       *p_pos);
}
   ...
   uint16_t * p_pos)
{
   ...
   pf_put_uint16 (
      ...
      p_pos);
      // ESP_LOGE("Ezjiillll",
      //    "MAC/PHY TLV: autoneg=0x%02X adv=0x%04X mau=0x%04X pos=%d",
      //    autoneg_status,
      //    p_status->autonegotiation_advertised_capabilities,
      //    p_status->operational_mau_type,
      //    *p_pos);   //ezhilLLDP
...
/**
 * @internal
 * Insert the optional System Capabilities TLV into a buffer.
 *
 * Required by some Profinet controllers (e.g. Siemens).
 * We advertise "Station Only" (bit 7 = 0x0080), enabled = same.
 *
 * TLV type = 7, length = 4
 *   2 bytes: system capabilities bitmap
 *   2 bytes: enabled capabilities bitmap
 *
 * @param p_buf   InOut: The buffer.
 * @param p_pos   InOut: The buffer position.
 */
#define LLDP_TYPE_SYSTEM_CAPABILITIES 7
static void pf_lldp_add_system_capabilities (uint8_t * p_buf, uint16_t * p_pos)
{
   const uint16_t caps = 0x0080; /* Station Only */
   pf_lldp_add_tlv_header (p_buf, p_pos, LLDP_TYPE_SYSTEM_CAPABILITIES, 4);
   pf_put_uint16 (true, caps, PF_FRAME_BUFFER_SIZE, p_buf, p_pos);
   pf_put_uint16 (true, caps, PF_FRAME_BUFFER_SIZE, p_buf, p_pos);
}
...
{
   /* Total TLV payload:
    *   1  mgmt addr string length field
    *   1  mgmt addr subtype
    *   4  IPv4 address
    *   1  interface number subtype
    *   4  interface number value
    *   1  OID length field (= 0, no OID)
    * ──────────────────
    *  12  total payload → matches working frame */
   const uint8_t tlv_payload_len = (uint8_t)(1 + 1 + p_man_address->len
                                              + 1 + 4 + 1);

   /* TLV header: type=8 */
   pf_lldp_add_tlv_header (p_buf, p_pos, LLDP_TYPE_MANAGEMENT_ADDRESS,
                            tlv_payload_len);

   /* Management address string length = subtype byte (1) + addr bytes (4) */
   pf_put_byte ((uint8_t)(1u + p_man_address->len),
                PF_FRAME_BUFFER_SIZE, p_buf, p_pos);

   /* Subtype: 1 = IPv4 */
   pf_put_byte (p_man_address->subtype,
                PF_FRAME_BUFFER_SIZE, p_buf, p_pos);

   /* IPv4 address (4 bytes) */
   pf_put_mem (p_man_address->value, p_man_address->len,
               PF_FRAME_BUFFER_SIZE, p_buf, p_pos);

   /* Interface number subtype: 2 = ifIndex */
   pf_put_byte (p_man_address->interface_number.subtype,
                PF_FRAME_BUFFER_SIZE, p_buf, p_pos);

   /* Interface number value = 1 */
   pf_put_uint32 (true, 1u, PF_FRAME_BUFFER_SIZE, p_buf, p_pos);

   /* OID length = 0, no OID bytes */
   pf_put_byte (0, PF_FRAME_BUFFER_SIZE, p_buf, p_pos);
}
static void pf_lldp_add_management_ez (
   const pf_lldp_management_address_t * p_man_address,
   uint8_t * p_buf,
   uint16_t * p_pos)
{
...
void pf_lldp_get_chassis_id_old(pnet_t * net, pf_lldp_chassis_id_t * p_chassis_id)
{
...
void pf_lldp_get_chassis_id_1 (pnet_t * net, pf_lldp_chassis_id_t * p_chassis_id)
{
   const pnet_ethaddr_t * device_mac_address =
      pf_cmina_get_device_macaddr (net);

   memset(p_chassis_id->string, 0, sizeof(p_chassis_id->string)); // 🔥 critical
   ...
   /* 🔥 FIX: enforce real length */
   p_chassis_id->len = strnlen(
      p_chassis_id->string,
      sizeof(p_chassis_id->string));

   /* 🔥 FIX: remove embedded NULLs */
   for (size_t i = 0; i < p_chassis_id->len; i++)
   {
      if (p_chassis_id->string[i] == '\0')
      {
         p_chassis_id->len = i;
         break;
      }
...
void pf_lldp_get_chassis_id (pnet_t * net, pf_lldp_chassis_id_t * p_chassis_id)   //ezhil
{
   const pnet_ethaddr_t * device_mac_address =
      pf_cmina_get_device_macaddr (net);
   char station_name[PNET_STATION_NAME_MAX_SIZE];

   if (  net->pf_interface.name_of_device_mode.mode ==PF_LLDP_NAME_OF_DEVICE_MODE_LEGACY)
   {
      // ESP_LOGE("Ezhillll...","DeviceModeLegacy");
      pf_cmina_get_station_name (net, station_name);

      size_t len = strnlen(station_name, sizeof(station_name));
      // ESP_LOGE("Ezhillll...","len==%d",len);

      if (
         len == 0 ||
         len >= PNET_LLDP_CHASSIS_ID_MAX_SIZE)
      {
         // ESP_LOGE("Ezhillll...","funcName==%s,Line==%d",__func__,__LINE__);
         /* Use MAC address */
         p_chassis_id->subtype = PF_LLDP_SUBTYPE_MAC;
         p_chassis_id->len = sizeof(pnet_ethaddr_t);
      ...
      else
      {
        // ESP_LOGE("Ezhillll...","funcName==%s,Line==%d",__func__,__LINE__);
         /* Use station name */
         p_chassis_id->subtype = PF_LLDP_SUBTYPE_LOCALLY_ASSIGNED;
         memcpy(p_chassis_id->string, station_name, len);
         p_chassis_id->len = len;
      }
   }
  else
{
...
{
   /* Working frame uses "po-00N" NOT "port-00N"
    * "po-002" = 70 6F 2D 30 30 32 (no 72 74) */
   snprintf (p_port_id->string, sizeof (p_port_id->string),
             "port-%03d", loc_port_num);
   p_port_id->subtype  = 7;
   p_port_id->len      = strlen (p_port_id->string); /* = 6 */
   p_port_id->is_valid = true;
}
void pf_lldp_get_port_id_oldd (
   pnet_t * net,
   int loc_port_num,
   pf_lldp_port_id_t * p_port_id)
{
   ...
   pf_lldp_link_status_t * p_link_status)
{
   ...
   /* Override all three IEEE 802.3 MAC/PHY fields to fixed values that
    * match what the Siemens reference frame advertises:
    *   autoneg byte  = 0x03  (supported | enabled)
    *   adv_caps      = 0x0400  (100BASE-TX Full Duplex only)
    *   mau_type      = read from PHY (0x0010 = 100BASE-TX FD)
    * The ESP32 PHY may report is_autoneg_supported/enabled as false
    * depending on link state; force them true to match the working frame. */
   p_link_status->is_autonegotiation_supported = true;
   p_link_status->is_autonegotiation_enabled   = true;
   p_link_status->autonegotiation_advertised_capabilities = 0x0400;
   ...
   size_t system_description_max_len)
{
   ...
   //ESP_LOGE("Ezhillll...","funcName==%s,Line==%d",__func__,__LINE__);
   ESP_LOGE("LLDP_DEBUG", "product_name                = [%s]", net->fspm_cfg.product_name);
ESP_LOGE("LLDP_DEBUG", "im_order_id                 = [%s]", net->fspm_cfg.im_0_data.im_order_id);
ESP_LOGE("LLDP_DEBUG", "im_serial_number            = [%s]", net->fspm_cfg.im_0_data.im_serial_number);
ESP_LOGE("LLDP_DEBUG", "im_hardware_revision        = [%u]", net->fspm_cfg.im_0_data.im_hardware_revision);

ESP_LOGE("LLDP_DEBUG", "im_sw_revision_prefix       = [%c]", net->fspm_cfg.im_0_data.im_sw_revision_prefix);
ESP_LOGE("LLDP_DEBUG", "im_sw_revision_func_enh     = [%u]", net->fspm_cfg.im_0_data.im_sw_revision_functional_enhancement);
ESP_LOGE("LLDP_DEBUG", "im_sw_revision_bug_fix      = [%u]", net->fspm_cfg.im_0_data.im_sw_revision_bug_fix);
ESP_LOGE("LLDP_DEBUG", "im_sw_revision_internal     = [%u]", net->fspm_cfg.im_0_data.im_sw_revision_internal_change);
   int ret = snprintf (
      system_description,
   ...
   /* 🔥 CRITICAL FIX */
   size_t len = strnlen(system_description, system_description_max_len);

   /* Remove trailing spaces */
   while (len > 0 && system_description[len - 1] == ' ')
   {
      len--;
   }

   system_description[len] = '\0';

   return len;
}
size_t pf_lldp_get_system_description_old (
   const pnet_t * net,
   char * system_description,
   size_t system_description_max_len)
{
...
static void pf_lldp_add_system_description (uint8_t * p_buf, uint16_t * p_pos)
{
   static const char sysDescr[] = "GENERAL";
   pf_lldp_add_tlv_header (p_buf, p_pos, LLDP_TYPE_SYSTEM_DESCRIPTION,
                            (uint8_t)strlen(sysDescr));
   pf_put_mem ((const uint8_t*)sysDescr, strlen(sysDescr),
               PF_FRAME_BUFFER_SIZE, p_buf, p_pos);
}
 ...
 * 
 */
/* ── helper: write one TLV header (type=7 bits, len=9 bits) ── */
static void put_tlv_hdr(uint8_t ** pp, uint8_t type, uint16_t len)
{
   uint16_t h = ((uint16_t)type << 9) | (len & 0x1FF);
   *(*pp)++ = (uint8_t)(h >> 8);
   *(*pp)++ = (uint8_t)(h & 0xFF);
}
...
{
   const pnet_ethaddr_t * device_mac_address = pf_cmina_get_device_macaddr (net);
   const pf_port_t * p_port_data = pf_port_get_state (net, loc_port_num);
   uint8_t * p = buf;
   uint8_t chassis_len;
   uint8_t port_len;
   ...
   char port_str[16];
   const char * chassis_str = "esp32-pn-dev1";   /* digit 1, NOT letter l */
   const char * sys_desc    = "GENERAL";

   /* ── Ethernet header ── */
   static const uint8_t lldp_dst[6] = {0x01,0x80,0xC2,0x00,0x00,0x0E};
   memcpy(p, lldp_dst, 6);                             p += 6;
   memcpy(p, p_port_data->netif.mac_address.addr, 6);  p += 6;
   *p++ = 0x88; *p++ = 0xCC;

   /* ── TLV 1: Chassis ID ── */
   chassis_len = (uint8_t)strlen(chassis_str);
   put_tlv_hdr(&p, 1, (uint16_t)(1 + chassis_len));
   *p++ = 0x07;                                        /* subtype: locally assigned */
   memcpy(p, chassis_str, chassis_len); p += chassis_len;

   /* ── TLV 2: Port ID ── */
   snprintf(port_str, sizeof(port_str), "port-%03d", loc_port_num);
   port_len = (uint8_t)strlen(port_str);
   put_tlv_hdr(&p, 2, (uint16_t)(1 + port_len));
   *p++ = 0x07;                                        /* subtype: locally assigned */
   memcpy(p, port_str, port_len); p += port_len;
   ...
   *p++ = 0x00; *p++ = 0x14;
   ...
   desc_len = (uint8_t)strlen(sys_desc);
   put_tlv_hdr(&p, 6, desc_len);
   memcpy(p, sys_desc, desc_len); p += desc_len;

   /* ── TLV 7: System Capabilities — Station Only ── */
   put_tlv_hdr(&p, 7, 4);
   *p++ = 0x00; *p++ = 0x80;   /* capabilities: Station */
   *p++ = 0x00; *p++ = 0x80;   /* enabled:      Station */

   /* ── TLV 8: Management Address ── */
   ipaddr = pf_cmina_get_ipaddr(net);
   ipaddr = CC_TO_BE32(ipaddr);
   put_tlv_hdr(&p, 8, 12);
   *p++ = 0x05;                 /* mgmt addr string length = 1(subtype)+4(IP) */
   *p++ = 0x01;                 /* subtype: IPv4 */
   memcpy(p, &ipaddr, 4); p += 4;
   *p++ = 0x02;                 /* interface number subtype: ifIndex */
   *p++ = 0x00; *p++ = 0x00; *p++ = 0x00; *p++ = 0x01;  /* ifIndex = 1 */
   *p++ = 0x00;                 /* OID length = 0 */

   /* ── TLV org-spec: PNIO Port Status (OUI=00:0E:CF sub=0x02) ── */
   put_tlv_hdr(&p, 127, 8);
   *p++ = 0x00; *p++ = 0x0E; *p++ = 0xCF;
   *p++ = 0x02;                 /* subtype: port status */
   *p++ = 0x00; *p++ = 0x00;   /* RTClass2 = OFF */
   *p++ = 0x00; *p++ = 0x00;   /* RTClass3 = OFF */

   /* ── TLV org-spec: PNIO Chassis MAC (OUI=00:0E:CF sub=0x05) ── */
   put_tlv_hdr(&p, 127, 10);
   *p++ = 0x00; *p++ = 0x0E; *p++ = 0xCF;
   *p++ = 0x05;                 /* subtype: chassis MAC */
   memcpy(p, device_mac_address->addr, 6); p += 6;

   /* ── TLV org-spec: IEEE 802.3 MAC/PHY (OUI=00:12:0F sub=0x01) ── */
   put_tlv_hdr(&p, 127, 9);
   *p++ = 0x00; *p++ = 0x12; *p++ = 0x0F;
   *p++ = 0x01;                 /* subtype: MAC/PHY config */
   *p++ = 0x03;                 /* autoneg: supported + enabled */
   *p++ = 0x04; *p++ = 0x00;   /* adv caps: 100BASE-TX FD */
   *p++ = 0x00; *p++ = 0x10;   /* MAU type: 100BaseTXFD */

   /* ── End TLV ── */
   *p++ = 0x00; *p++ = 0x00;

   ESP_LOGE("LLDP", "construct frame len=%d port=%d", (int)(p - buf), loc_port_num);
   return (size_t)(p - buf);
}

size_t pf_lldp_construct_frameeeee (pnet_t * net, int loc_port_num, uint8_t buf[])
{
   uint16_t pos;
   const pnet_ethaddr_t * device_mac_address =
      pf_cmina_get_device_macaddr (net);
...
//ESP_LOGE("Ezhil....","pfLLdp.c");
   ...
   pf_lldp_add_system_capabilities (buf, &pos);
   pf_lldp_add_system_description (buf, &pos);   // ← ADD THIS
  ...
  pf_lldp_add_ieee_mac_phy (&link_status, buf, &pos);
   ...
   //ESP_LOGE(TAG, "LLDP FINAL LENGTH = %d", *p_pos);
 ...
 */
uint8_t lldp_frame[] = {
    0x01,0x80,0xC2,0x00,0x00,0x0E,0x14,0xB1,
    0x26,0xB1,0xD0,0x19,0x88,0xCC,0x02,0x13,
    0x07,0x63,0x6F,0x6E,0x76,0x65,0x79,0x6C,
    0x69,0x6E,0x78,0x70,0x6C,0x63,0x2D,0x31,
    0x2D,0x32,0x30,
    0x04,0x09,0x07,0x70,0x6F,0x2D,0x30,0x30,0x32,
    0x06,0x02,0x00,0x14,
    0x10,0x14,0x05,0x01,0xC0,0xA8,0x01,0x14,
    0x02,0x00,0x00,0x00,0x01,0x08,
    0x2B,0x06,0x01,0x04,0x01,0x81,0xC0,0x6E,
    0x02,0x07,0x47,0x45,0x4E,0x45,0x52,0x41,0x4C,
    0x0E,0x04,0x00,0x80,0x00,0x80,
    0xFE,0x08,0x00,0x0E,0xCF,0x02,0x00,0x00,0x00,
    0x00,0xFE,0x0A,0x00,0x0E,0xCF,0x05,0x14,0xB1,
    0x26,0x07,0x25,0x6F,
    0xFE,0x09,0x00,0x12,0x0F,0x01,0x03,0x04,0x00,
    0x00,0x10,0x00,0x00
};

 static void pf_lldp_send_new (pnet_t * net, int loc_port_num)
{
   pnal_buf_t * p_buffer = pnal_buf_alloc(PF_FRAME_BUFFER_SIZE);

   ESP_LOGE("LLDP", "Sending hardcoded packet");
   ...
   {
      /* 🔴 Your hardcoded LLDP frame */
      // uint8_t lldp_frame[] = {
      //    0x01,0x80,0xC2,0x00,0x00,0x0E,0x14,0xB1,
      //    0x26,0xB1,0xD0,0x19,0x88,0xCC,0x02,0x13,
      //    0x07,0x63,0x6F,0x6E,0x76,0x65,0x79,0x6C,
      //    0x69,0x6E,0x78,0x70,0x6C,0x63,0x2D,0x31,
      //    0x2D,0x32,0x30,0x04,0x09,0x07,0x70,0x6F,
      //    0x2D,0x30,0x30,0x32,0x06,0x02,0x00,0x14,
      //    0x10,0x14,0x05,0x01,0xC0,0xA8,0x01,0x14,
      //    0x02,0x00,0x00,0x00,0x01,0x08,0x2B,0x06,
      //    0x01,0x04,0x01,0x81,0xC0,0x6E,0x02,0x07,
      //    0x47,0x45,0x4E,0x45,0x52,0x41,0x4C,0x0E,
      //    0x04,0x00,0x80,0x00,0x80,0xFE,0x08,0x00,
      //    0x0E,0xCF,0x02,0x00,0x00,0x00,0x00,0xFE,
      //    0x0A,0x00,0x0E,0xCF,0x05,0x14,0xB1,0x26,
      //    0x07,0x25,0x6F,0xFE,0x09,0x00,0x12,0x0F,
      //    0x01,0x03,0x04,0x00,0x00,0x10,0x00,0x00
      // };

      uint16_t frame_len = sizeof(lldp_frame);

      /* Copy into buffer */
      memcpy(p_buffer->payload, lldp_frame, frame_len);
      p_buffer->len = frame_len;

     // ESP_LOGE("EzhillllllllllllllLLDP", "Length = %d", p_buffer->len);
      print_buffer((uint8_t *)p_buffer->payload, p_buffer->len);
...
static void pf_lldp_send (pnet_t * net, int loc_port_num)
{
  ...
  // ESP_LOGI ("LLDP", "pf_lldp_send: port=%d", loc_port_num);

   /* NULL check MUST come before any use of p_buffer */
   if (p_buffer == NULL)
   {
      ESP_LOGE ("LLDP", "pf_lldp_send: buffer alloc failed");
      return;
   }

   if (p_buffer->payload != NULL)
   {
      p_buffer->len = pf_lldp_construct_frame (net, loc_port_num, p_buffer->payload);
     // ESP_LOGI ("LLDP", "pf_lldp_send: port=%d len=%d", loc_port_num, p_buffer->len);
     // print_buffer ((uint8_t *)p_buffer->payload, p_buffer->len);
      (void)pf_eth_send_on_physical_port (net, loc_port_num, p_buffer); /* send only once */
   }
 ...
 * @param current_time     In:    Not used.
 */
// #define LLDP_TYPE_SYSTEM_DESCRIPTION  5   /* Wireshark: SysD= */
#define LLDP_TYPE_SYSTEM_NAME         6   /* Wireshark: SysN= */
static void pf_lldp_trigger_sending (
   pnet_t * net,
   void *   arg,
   uint32_t current_time)
{
   pf_port_t * p_port_data = (pf_port_t *)arg;  /* single declaration */

   // ESP_LOGI ("LLDP", "trigger_sending: port=%d", p_port_data->port_num);

   /* Send on ALL ports — no 'if (port_num == 2)' gate */
   if (p_port_data->port_num == 1)
   {
