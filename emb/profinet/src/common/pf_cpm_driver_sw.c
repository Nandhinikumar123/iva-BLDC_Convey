   ...
   uint32_t current_time)
{
   ...
   //       ESP_LOGE("CPM", "RX frame len=%u buf_len=%u transfer=%u",
   //          len, p_cpm->buffer_length, transfer_status);
   // ESP_LOGE("CPM", "frame_ok=%d c_sdu_ok=%d dht=%u dhf=%u",
   //          frame_structure, c_sdu_structure,
   //          p_iocr->cpm.dht, p_iocr->cpm.data_hold_factor);
...
//ESP_LOGE("CPM", "Frame received: len=%u state=%d", p_buf->len, p_cpm->state);
      dht_reload =
         (frame_structure && c_sdu_structure && (data_valid || backup));
      update_data =
         (frame_structure && c_sdu_structure && data_valid &&
          (primary || backup));
// //ESP_LOGE("CPM", "frame_structure=%d c_sdu=%d len=%u buf_len=%u transfer=%u dht=%u",
//          frame_structure, c_sdu_structure, len, p_cpm->buffer_length,
//          transfer_status, p_iocr->cpm.dht);
      if (data_valid == false)
      {
         /* 19 */
         /* Ignore */
