...
void pf_scheduler_remove (pnet_t * net, pf_scheduler_handle_t * handle)
{
   ...
   if (handle->timer_index == 0 || handle->timer_index > PF_MAX_TIMEOUTS)
   {
      LOG_ERROR (
         PNET_LOG,
         "SCHEDULER(%d): Invalid value %" PRIu32 " for timeout \"%s\". No removal.\n",
         __LINE__,
         handle->timer_index,
         SAFE_STR(handle->name));   // ✅ FIX
   }
   else
   {
      ...
      if (net->scheduler_timeouts[ix].name != handle->name)
      {
         LOG_ERROR (
            ...
            __LINE__,
            SAFE_STR(net->scheduler_timeouts[ix].name),   // ✅ FIX
            SAFE_STR(handle->name));                      // ✅ FIX
      }
      else if (net->scheduler_timeouts[ix].in_use == false)
      {
         LOG_DEBUG (
            PNET_LOG,
            "SCHEDULER(%d): Tried to remove timeout \"%s\", but it has already been triggered.\n",
            __LINE__,
            SAFE_STR(handle->name));   // ✅ FIX
      }
      else
...
void pf_scheduler_remove_ez(pnet_t * net, pf_scheduler_handle_t * handle)
{
   uint16_t ix;
