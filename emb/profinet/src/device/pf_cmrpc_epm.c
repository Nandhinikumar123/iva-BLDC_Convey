...
#include "pnal.h"   /* pnal_get_ip_address() */
   ...
   uint16_t udp_port)
{
   ...
   /* Floor 5 IP Address — must be the device's actual IP, not 0.0.0.0.
    * If 0 is sent, the PLC ignores the EPM response and keeps retrying. */
   p_tower->floor_5.protocol_id = PF_RPC_PROTOCOL_DOD_IP;
   p_tower->floor_5.ip_address = htonl (pnal_get_ip_address (
      net->fspm_cfg.if_cfg.main_netif_name));
}
