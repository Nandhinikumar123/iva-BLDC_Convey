   ...
   pnet_result_t * p_stat)
{
   ...
   for (ix = 0; ix < p_ar->nbr_iocrs; ix++)
   {
      ...
      /* Diagnostic dump: log all IOCR parameters from the PLC Connect request */
      LOG_INFO (PNET_LOG,
         "CMDEV_IOCR[%u]: type=%u rt_class=%u lt=0x%04X "
         "sdu=%u clk=%u rr=%u ph=%u "
         "dhf=%u vlan=%u prio=%u fid=0x%04X "
         "mdi=%d clk_x_rr=%u\n",
         (unsigned)ix,
         (unsigned)p_iocr->iocr_type,
         (unsigned)p_iocr->iocr_properties.rt_class,
         (unsigned)p_iocr->lt_field,
         (unsigned)p_iocr->c_sdu_length,
         (unsigned)p_iocr->send_clock_factor,
         (unsigned)p_iocr->reduction_ratio,
         (unsigned)p_iocr->phase,
         (unsigned)p_iocr->data_hold_factor,
         (unsigned)p_iocr->iocr_tag_header.vlan_id,
         (unsigned)p_iocr->iocr_tag_header.iocr_user_priority,
         (unsigned)p_iocr->frame_id,
         (int)pf_fspm_get_min_device_interval(net),
         (unsigned)(p_iocr->send_clock_factor * p_iocr->reduction_ratio));
   ...
   pnet_result_t * p_stat)
{
   ...
   if (p_ar != NULL)
   {
      ...
      {
         // LOG_INFO (PNET_LOG,
         //    "CMDEV: Connect RPC received. nbr_ar_param=%u nbr_iocrs=%u "
         //    "nbr_exp_apis=%u nbr_alarm_cr=%u\n",
         //    (unsigned)p_ar->nbr_ar_param,
         //    (unsigned)p_ar->nbr_iocrs,
         //    (unsigned)p_ar->nbr_exp_apis,
         //    (unsigned)p_ar->nbr_alarm_cr);  //Ezhil RPC........
         ret = pf_cmdev_check_ar_param (p_ar, p_stat);
         if (ret != 0)
         {
            LOG_ERROR (PNET_LOG,
               "CMDEV: check_ar_param FAILED: ec=0x%02X ec1=0x%02X ec2=0x%02X "
               "ar_type=%u ar_state=%u startup_mode=%u dev_access=%u "
               "param_server=%u\n",
               (unsigned)p_stat->pnio_status.error_code,
               (unsigned)p_stat->pnio_status.error_code_1,
               (unsigned)p_stat->pnio_status.error_code_2,
               (unsigned)p_ar->ar_param.ar_type,
               (unsigned)p_ar->ar_param.ar_properties.state,
               (unsigned)p_ar->ar_param.ar_properties.startup_mode,
               (unsigned)p_ar->ar_param.ar_properties.device_access,
               (unsigned)p_ar->ar_param.ar_properties.parameterization_server);
         }
      }
      ...
      if (ret == 0)
      {
         ...
         {
            LOG_ERROR (PNET_LOG,
               "CMDEV: check_iocr_param FAILED: error_code=0x%02X "
               "error_code_1=0x%02X error_code_2=0x%02X\n",
               p_stat->pnio_status.error_code,
               p_stat->pnio_status.error_code_1,
      ...
      if (ret == 0)
      {
         ...
         {
            LOG_ERROR (PNET_LOG,
               "CMDEV: exp_apis_configure FAILED: error_code=0x%02X "
               "error_code_1=0x%02X error_code_2=0x%02X\n",
               p_stat->pnio_status.error_code,
               p_stat->pnio_status.error_code_1,
               p_stat->pnio_status.error_code_2);
