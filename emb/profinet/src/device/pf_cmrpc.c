...
{
   int ret = -1;
   char ip_string[PNAL_INET_ADDRSTR_SIZE] = {0}; 
   ...
   if (size != 0)
   {
      ...
      //    __LINE__, size, (unsigned)p_sess->socket, ip_string, p_sess->port,
      //    payload_description, p_sess->from_me, p_sess->ix);    //Sending Bytes

      if (
         pf_udp_sendto (
            p_net, p_sess->socket, p_sess->ip_addr, p_sess->port, data, size) == size)
      {
         /* --- ESP32 NETWORK SNIFFER: OUTGOING --- */
         // ESP_LOGI("PN_SNIFFER", ">>> OUTGOING RPC FRAME (%s) [%d bytes] >>>", payload_description, size);
         // ESP_LOG_BUFFER_HEXDUMP("PN_SNIFFER", data, size, ESP_LOG_INFO);
         /* --------------------------------------- */
         ret = 0;
      }
   }
   ...
   {
      LOG_DEBUG (PF_RPC_LOG, "CMRPC(%d): No UDP data to send on the socket.\n", __LINE__);
   }
   ...
   else
   {
         ...
         (rpc_req.flags.last_fragment == true))
      {
         ...
         case PF_RPC_PT_REQUEST:
            ...
            // /* Log which interface UUID was received */
            // LOG_INFO (PF_RPC_LOG,
            //    "CMRPC: RPC opnum=%u seqnmb=%u uuid=%08X-%04X-%04X "
            //    "is_epm=%u is_pnio=%u\n",
            //    (unsigned)rpc_req.opnum,
            //    (unsigned)rpc_req.sequence_nmb,
            //    (unsigned)rpc_req.interface_uuid.data1,
            //    (unsigned)rpc_req.interface_uuid.data2,
            //    (unsigned)rpc_req.interface_uuid.data3,
            //    (unsigned)(memcmp(&rpc_req.interface_uuid, &uuid_epmap_interface, sizeof(rpc_req.object_uuid)) == 0),
            //    (unsigned)(memcmp(&rpc_req.interface_uuid, &uuid_io_device_interface, sizeof(rpc_req.object_uuid)) == 0));
            if (
               memcmp (
...
void pf_cmrpc_periodic (pnet_t * net)
{
   ...
   {
      if ((net->cmrpc_session_info[ix].in_use == true) &&
          (net->cmrpc_session_info[ix].from_me == true))
      {
         dcerpc_input_len = pf_udp_recvfrom (
            net, net->cmrpc_session_info[ix].socket, &dcerpc_addr, &dcerpc_port,
            net->cmrpc_dcerpc_input_frame, sizeof (net->cmrpc_dcerpc_input_frame));
            
         if (dcerpc_input_len > 0)
         {
            /* --- ESP32 NETWORK SNIFFER: INCOMING SESSION --- */
            // ESP_LOGI("PN_SNIFFER", "<<< INCOMING RPC FRAME (Session %u) [%d bytes] <<<", ix, dcerpc_input_len);
            // ESP_LOG_BUFFER_HEXDUMP("PN_SNIFFER", net->cmrpc_dcerpc_input_frame, dcerpc_input_len, ESP_LOG_INFO);
            // /* ----------------------------------------------- */
            ...
            (void)pf_cmrpc_dce_packet (
               ...
               dcerpc_input_len, net->cmrpc_dcerpc_output_frame, &dcerpc_resp_len, &close_socket);
   ...
   /* Poll New RPC requests */
   dcerpc_input_len = pf_udp_recvfrom (
      net, net->cmrpc_rpcreq_socket, &dcerpc_addr, &dcerpc_port,
      net->cmrpc_dcerpc_input_frame, sizeof (net->cmrpc_dcerpc_input_frame));
      
   if (dcerpc_input_len > 0)
   {
      /* --- ESP32 NETWORK SNIFFER: INCOMING REQUEST --- */
      // ESP_LOGI("PN_SNIFFER", "<<< INCOMING RPC FRAME (New Req) [%d bytes] <<<", dcerpc_input_len);
      // ESP_LOG_BUFFER_HEXDUMP("PN_SNIFFER", net->cmrpc_dcerpc_input_frame, dcerpc_input_len, ESP_LOG_INFO);
      /* ----------------------------------------------- */
