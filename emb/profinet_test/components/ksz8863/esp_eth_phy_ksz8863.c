...
{
    //ESP_LOGE("Ezhillllllll","val=%s","Helloworlddd");
    esp_err_t ret = ESP_OK;
    phy_ksz8863_t *ksz8863 = __containerof(phy, phy_ksz8863_t, parent);
    esp_eth_mediator_t *eth = ksz8863->eth;
//ESP_LOGE("Ezhillllllll","val=%d",ksz8863->driver_mode);
    if (ksz8863->driver_mode == KSZ8863_MAC_MAC_MODE) {
        return ESP_OK;
    }

    ksz8863_pcr13_reg_t pcr13;
    ESP_LOGI("PHYyyyyyyyyyyyy","addr=%d", ksz8863->addr);
    ESP_LOGI("PHYyyyyyyyyyyyy","offset=%d", ksz8863->port_reg_offset);
    ...
    if (!enable) {
        ...
        ESP_LOGI("Ezhil","%d",ksz8863->reset_timeout_ms);
        ksz8863->reset_timeout_ms=1000;
        ESP_GOTO_ON_FALSE(to < ksz8863->reset_timeout_ms / 10, ESP_FAIL, err, TAG, "power up timeoutttt");
    }
    return ESP_OK;
err:
    return ret;
...
static esp_err_t ksz8863_init(esp_eth_phy_t *phy)
{
    ...
    ESP_GOTO_ON_ERROR(ksz8863_pwrctl(phy, true), err, TAG, "power control failedddddddddddd");
    /* Check PHY ID */
    ...
    ESP_LOGE("addressss","%d",ksz8863->addr);
    ESP_LOGE("Ezhilll....","%x",id0.val);
    ESP_LOGE("Ezhilll....","%d",id1.val);
    ESP_GOTO_ON_FALSE(id0.family_id == 0x88 && id1.chip_id == 0x3, ESP_FAIL, err, TAG, "wrong chip IDfffffffffffffffff");
